...
# 相对路径
file_path = "configer_loudao.json"
...
car_coor_final = np.array([0.31893438, 9.66071406, -0.2041373])
# car_coor_final = pos_b_l[0:3]
print(car_coor_final)
angle_ins = np.array([359.04, 0.45, -1.71])
...
NeToIns_mat = euler_matrix(angle_ins[::-1], False)
NeToRoad_mat = euler_matrix(angle_h_n[::-1], True)

car_coor_final_body = -np.dot(NeToIns_mat.T, np.dot(NeToRoad_mat.T, car_coor_final.reshape(3, 1))) - CarToCar2

car_coor_final = np.dot(NeToRoad_mat, np.dot(NeToIns_mat, -car_coor_final_body))
print(car_coor_final)
