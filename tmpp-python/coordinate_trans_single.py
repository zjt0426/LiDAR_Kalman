...
def LtoBtrans(angle_l_b, angle_i_b, angle_ins, angle_h_n, max_iter, out_lidar, pos_lidar, pos_lf):
    ...
    for i in range(max_iter):
        C_l2b = euler_matrix(angle_l_b[::-1],flag=False)
        #print('c-l2b')
        #print(C_l2b)
...
def ItoHtrans(jwg_ins, jwg_biaoba, max_iter, a, b, angle_i_b, angle_ins, angle_h_n,
    ...
    for i in range(max_iter):
        # e_2 = ((a*a - b*b)/(a*a))
        # N0 = a/((1-e_2**2*np.sin(math.radians(jwg_ins[1, i]))**2)**(1/2))
        ...
        # z0 = (N0*(1-e_2*e_2)+jwg_ins[2, i])*np.sin(math.radians(jwg_ins[1, i]))
        # N1 = a/((1-e_2**2*np.sin(math.radians(jwg_biaoba[1]))**2)**(1/2))
        ...
        # z1 = (N1*(1-e_2*e_2)+jwg_biaoba[2])*np.sin(math.radians(jwg_biaoba[1]))
        e_2 = ((a * a - b * b) / (a * a))
...
def cal_output(data, jwg_biaoba, angle_h_n, angle_l_b):
    max_iter = 1
    # 标定和坐标转换必要数据及参数
    # angle_l_b = np.array([-5.87, -4.85, 1.5])   # 雷达跟载体的角度，一号雷达标定角度
    angle_i_b = 0                               # 惯导跟载体x轴的角度
    angle_ins = data[:, 3:6].T
    # angle_h_n = np.array([0.15,	-0.2775,0.24])   # 巷道跟东北天的欧拉角
    out_lidar = data[:, -3:].T
    # print('out_lidar')
    # print(out_lidar)
    pos_lidar = np.array([0, 0, 0]).T   # 雷达原点在载体上的坐标(1号)
    pos_lf = np.zeros((3, 1))

    jwg_ins = data[:, 0:3].T        # 惯导经纬高

    # jwg_biaoba = np.array([108.6788481, 34.2224881, 332.6])    #标靶处的经纬高
    a = 6378137
    b = 6356752.31424
    # pos_ins = np.array([0.09,  0.285,      0]).T  # 列向量
    pos_ins = np.array([-0.9486637, -1.72215897, -1.09309636]).T  # 列向量
    pos_if = np.zeros((3, 1))
